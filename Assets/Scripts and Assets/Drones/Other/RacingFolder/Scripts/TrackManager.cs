...
public class TrackManager : MonoBehaviour
{
    ...
    private void Awake()
    {
		...
		//Showing best time
		if (PlayerPrefs.HasKey("FPV_D_C"))
		{
			if (bestLapTime_UI)
			{
				bestLapTime = PlayerPrefs.GetFloat("FPV_D_C");
				bestLapTime_UI.text = bestLapTime.ToString("f2") + " sec";
			}
		}		
	}

	public void DeleteBestLapTime()
	{
		if (PlayerPrefs.HasKey("FPV_D_C"))
		{
			PlayerPrefs.DeleteKey("FPV_D_C");
		}
	}
    ...
    void ActivateNextWayPoint()
    {
		...
		Transform neonePole = waypointArray[waypointCounter].Find("NeonPole");
		...
		MeshRenderer meshRenderer_NeonPole = neonePole.GetComponent<MeshRenderer>();
		...
		meshRenderer_NeonPole.materials[0].SetColor("_Color", activeWaypointColor);
		meshRenderer_NeonPole.materials[0].SetColor("_EmissionColor", activeWaypointColor);
    ...
    */
    ...
    void ActivateSecondNextWayPoint()
    {
		...
		Transform neonePole = waypointArray[secondNextWayPoint].Find("NeonPole");
		...
		meshRenderer_NeonPole.materials[0].SetColor("_Color", nextWaypointColor);
		meshRenderer_NeonPole.materials[0].SetColor("_EmissionColor", nextWaypointColor);
    ...
    void DeactivateCurrentWayPoint()
    {
		...
		meshRenderer_NeonPole.materials[0].SetColor("_Color", inactiveWaypointColor);
		meshRenderer_NeonPole.materials[0].SetColor("_EmissionColor", inactiveWaypointColor);
    ...
    void LapTimerMethod()
    {
        ...
        else if (waypointCounter == 0)
        {
            ...
            if (lapTime < bestLapTime)
            {
                ...
                bestLapTime = lapTime;
				PlayerPrefs.SetFloat("FPV_D_C", bestLapTime);
                if (bestLapTime_UI)
                {
